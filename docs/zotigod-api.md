...
Channel Session tools recheck the current connection owner on every call. Owners may list and read other active Sessions within the bound Workspace; other Channel senders may only list and read their bound Session. Owners may also fork these readable Sessions through a completed turn, subject to existing runtime, approval and prompt compatibility checks. Other write/control permissions are unchanged, and cross-Workspace reads or forks remain forbidden. Ownership is resolved from the stored connection and trusted input actor, never a model argument or claimed actor role.
...
Catalog schema 8 adds project/workspace position and pin columns plus a one-time import marker; schema 9 normalizes historical catalog structures for generated SQL migrations. Deploy the new daemon before the paired UI and update the UI repository's `daemon-version` to that daemon revision before release. Old binaries reject newer schemas. An explicit offline `zotigod catalog-migrate --root <root> --to 7` can prepare a rollback while preserving sessions, but discards the new project/workspace navigation preferences. Stop all daemons and take a consistent backup first. See [catalog migrations](catalog-migrations.md) for generation, version compatibility, dirty recovery and the old-daemon locking boundary. Old UI preferences are retained but no longer consulted after import.
...
Automatic reads and all writes use roots from registered sources, active workspaces and the optional session's stored working directory. `POST /files/open` additionally accepts `explicitOpen: true` for a user-initiated read outside those roots; Bearer authentication and operating-system permissions still apply. Text files opened outside registered roots are returned read-only. This flag is ignored by `/files/save` and does not grant directory browsing or write access. Clients must set it only following an explicit user open/load action, never from Markdown content alone. `imageOnly: true` restricts the response to image data; an automatic image request outside the allowed roots (or with an unresolved path) returns `data: {"kind":"requires_confirmation"}` without reading file contents. An explicit image read fails if the image is missing, unsupported or oversized. Clients targeting older daemons must not bypass a rejection by loading a local `file://` URL. Resolved symlinks must remain inside a registered root, and filesystem access uses a confined root handle. Missing or outside-root paths return 403. Invalid/uneditable files return 400. A stale modification time returns 409; clients should reopen before retrying. Save checks and writes are serialized across API clients of one daemon. This is not a lock against other programs editing files; external editors and agent tools remain independent. Saves modify existing files only, and are not a general upload or filesystem-management API. JSON bodies are bounded at 7 MiB, text reads at 5 MiB and edits at 1 MiB.
