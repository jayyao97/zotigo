...
func (h *handler) EnsureChannelSessionPrompt(ctx context.Context, sessionID string, prompt channels.SessionPromptConfig) (channels.SessionPromptConfig, error) {
	...
	if zotigoruntime.AgentKind(stored.Agent) != zotigoruntime.AgentZotigo {
		return channels.SessionPromptConfig{}, errors.New("channel prompts currently require a Zotigo session")
	}
...
func (h *handler) ProvisionChannelSession(ctx context.Context, workspaceID, title string, prompt channels.SessionPromptConfig, bind func(sessionID string) error) error {
	if h.store == nil {
	...
	profileName, _, err := appConfig.ResolveProfile("")
	if err != nil {
	...
	if err := bind(session.ID); err != nil {
...
func (h *handler) DispatchChannelTask(parent context.Context, task channels.Task, progress func(channels.Progress), admissionComplete func()) (channels.TaskResult, error) {
	...
	err := func() error {
		...
		if zotigoruntime.AgentKind(stored.Agent) != zotigoruntime.AgentZotigo {
			return errors.New("channel prompts currently require a Zotigo session")
		}
		if stored.ApprovalPolicy != agent.ApprovalPolicyAuto {
	...
	accepted, err := h.acceptSessionInputCommand(ctx, task.SessionID, commandID, task.Text, nil, nil, "", false, true, &sessionInputConstraints{approvalPolicy: agent.ApprovalPolicyAuto, promptRevision: desired.Revision, requestContext: requestContext})
	if err != nil {
	...
	for {
		result, done, waitErr := h.channelTaskResult(ctx, task.SessionID, after, &projected, &turnID, &approvalReported, progress)
		if done || waitErr != nil {
...
func (h *handler) channelTaskResult(ctx context.Context, sessionID string, after uint64, projected *uint64, turnID *string, approvalReported *bool, progress func(channels.Progress)) (channels.TaskResult, bool, error) {
	...
	for _, item := range items {
		...
		case zotigosession.DisplayItemTurnCompleted:
			...
			return channels.TaskResult{Text: answer}, true, nil
