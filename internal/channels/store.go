...
CREATE TABLE IF NOT EXISTS channel_conversations (
 ...
 chat_id TEXT NOT NULL, root_id TEXT NOT NULL DEFAULT '', thread_id TEXT NOT NULL DEFAULT '', chat_type TEXT NOT NULL DEFAULT 'group', chat_name TEXT NOT NULL DEFAULT '', display_name TEXT NOT NULL DEFAULT '',
...
(id,connection_id,conversation_id,provider_message_id,sender_id,sender_name,text,mentioned_bot,trigger_status,status_detail,reply_message_id,delivery_mode,cot_id,final_message_id,projected_sequence,created_at)
SELECT id,connection_id,conversation_id,provider_message_id,sender_id,sender_name,text,mentioned_bot,trigger_status,status_detail,reply_message_id,delivery_mode,cot_id,final_message_id,projected_sequence,created_at
...
func scanConversation(row interface{ Scan(...any) error }) (Conversation, error) {
	...
	err := row.Scan(&c.ID, &c.ConnectionID, &c.ChatID, &c.RootID, &c.ThreadID, &c.ChatType, &c.ChatName, &c.DisplayName, &c.SessionID, &c.WorkspaceID, &enabled, &allowed, &observed, &c.AgentInstructionsMode, &c.AgentInstructions, &c.ApprovalInstructionsMode, &c.ApprovalInstructions, &review, &last, &created, &updated)
	if errors.Is(err, sql.ErrNoRows) {
		return c, ErrNotFound
	}
	if err != nil {
...
const conversationColumns = `id,connection_id,chat_id,root_id,thread_id,chat_type,chat_name,display_name,session_id,workspace_id,enabled,allowed_sender_ids,observed_senders,agent_mode,agent_instructions,approval_mode,approval_instructions,review_all_tools,last_activity_at,created_at,updated_at`
...
func (s *Store) PutConversation(ctx context.Context, c Conversation) (Conversation, error) {
	...
	defer tx.Rollback()
	result, err := tx.ExecContext(ctx, `UPDATE channel_conversations SET chat_name=?,display_name=?,session_id=?,workspace_id=?,enabled=?,allowed_sender_ids=?,agent_mode=?,agent_instructions=?,approval_mode=?,approval_instructions=?,review_all_tools=?,updated_at=? WHERE id=?`, c.ChatName, c.DisplayName, c.SessionID, c.WorkspaceID, c.Enabled, jsonText(c.AllowedSenderIDs), c.AgentInstructionsMode, c.AgentInstructions, c.ApprovalInstructionsMode, c.ApprovalInstructions, c.ReviewAllTools, now.Format(time.RFC3339Nano), c.ID)
	if err != nil {
		return Conversation{}, err
	}
...
func (s *Store) RecordMessage(ctx context.Context, connectionID string, in InboundMessage) (Message, error) {
	...
	if in.ChatType == "group" && rootID != "" {
		...
		if err = tx.QueryRowContext(ctx, `SELECT id FROM channel_conversations WHERE connection_id=? AND chat_id=? AND root_id=''`, connectionID, in.ChatID).Scan(&groupConversationID); err != nil {
			return Message{}, err
		}
		if in.ChatName != "" {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO channel_conversations(id,connection_id,chat_id,root_id,thread_id,chat_type,chat_name,display_name,last_activity_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`, convID, connectionID, in.ChatID, rootID, in.ThreadID, in.ChatType, in.ChatName, in.ConversationName, at.Format(time.RFC3339Nano), at.Format(time.RFC3339Nano), at.Format(time.RFC3339Nano))
	} else if err == nil {
		_, err = tx.ExecContext(ctx, `UPDATE channel_conversations SET root_id=?,thread_id=CASE WHEN ?<>'' THEN ? ELSE thread_id END,chat_name=CASE WHEN ?<>'' THEN ? ELSE chat_name END,display_name=CASE WHEN display_name='' THEN ? ELSE display_name END,last_activity_at=?,updated_at=? WHERE id=?`, rootID, in.ThreadID, in.ThreadID, in.ChatName, in.ChatName, in.ConversationName, at.Format(time.RFC3339Nano), at.Format(time.RFC3339Nano), convID)
	}
	if err != nil {
		return Message{}, err
	}
	...
	m := Message{ID: "msg_" + connectionID + "_" + in.MessageID, ConnectionID: connectionID, ConversationID: convID, ProviderID: in.MessageID, Sender: in.Sender, Text: in.Text, MentionedBot: in.MentionedBot, TriggerStatus: "received", CreatedAt: at}
	_, err = tx.ExecContext(ctx, `INSERT INTO channel_messages(id,connection_id,conversation_id,provider_message_id,sender_id,sender_name,text,mentioned_bot,trigger_status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(connection_id,provider_message_id) DO NOTHING`, m.ID, connectionID, convID, in.MessageID, in.Sender.ID, in.Sender.DisplayName, in.Text, in.MentionedBot, m.TriggerStatus, at.Format(time.RFC3339Nano))
	if err != nil {
		return Message{}, err
	}
...
const messageColumns = `id,connection_id,conversation_id,provider_message_id,sender_id,sender_name,text,mentioned_bot,trigger_status,status_detail,reply_message_id,delivery_mode,cot_id,final_message_id,projected_sequence,created_at`
...
func scanMessage(row interface{ Scan(...any) error }) (Message, error) {
	...
	var projected int64
	var created string
	err := row.Scan(&m.ID, &m.ConnectionID, &m.ConversationID, &m.ProviderID, &m.Sender.ID, &m.Sender.DisplayName, &m.Text, &mentioned, &m.TriggerStatus, &m.StatusDetail, &m.ReplyMessageID, &m.DeliveryMode, &m.COTID, &m.FinalMessageID, &projected, &created)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) SetMessageRunning(ctx context.Context, id string, receipt DeliveryReceipt) error {
	result, err := s.db.ExecContext(ctx, `UPDATE channel_messages SET trigger_status='running',status_detail='',reply_message_id=?,delivery_mode=?,cot_id=? WHERE id=? AND trigger_status='claimed'`, receipt.MessageID, receipt.Mode, receipt.COTID, id)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
...
func (s *Store) SetMessageDeliveryUnknown(ctx context.Context, id string, receipt DeliveryReceipt, finalMessageID, detail string) error {
	result, err := s.db.ExecContext(ctx, `UPDATE channel_messages SET trigger_status='delivery_unknown',status_detail=?,reply_message_id=?,delivery_mode=?,cot_id=?,final_message_id=? WHERE id=? AND trigger_status IN ('claimed','running')`, detail, receipt.MessageID, receipt.Mode, receipt.COTID, finalMessageID, id)
	if err != nil {
		return err
	}
	affected, err := result.RowsAffected()
...
func scanMessages(rows *sql.Rows) ([]Message, error) {
	...
	for rows.Next() {
		...
		var projected int64
		var created string
		if err := rows.Scan(&m.ID, &m.ConnectionID, &m.ConversationID, &m.ProviderID, &m.Sender.ID, &m.Sender.DisplayName, &m.Text, &mentioned, &m.TriggerStatus, &m.StatusDetail, &m.ReplyMessageID, &m.DeliveryMode, &m.COTID, &m.FinalMessageID, &projected, &created); err != nil {
			return nil, err
		}
		m.MentionedBot = mentioned != 0
		if projected > 0 {
