...
type userInputBoundary uint8

const (
	userInputContinuesTurn userInputBoundary = iota
	userInputStartsTurn
)
...
func (a *Agent) RunMessage(ctx context.Context, msg protocol.Message) (<-chan protocol.Event, error) {
	...
	if msg.Role != "" {
		...
		if msg.Role == protocol.RoleUser {
			if err := a.appendUserInputLocked(msg, userInputStartsTurn); err != nil {
				a.mu.Unlock()
				return nil, err
			}
...
func (a *Agent) drainPendingTurnUserInputLocked(pendingCount int) (bool, error) {
	...
	if err := a.appendUserInputLocked(msg, userInputContinuesTurn); err != nil {
		return false, err
	}
...
func (a *Agent) appendUserInputLocked(msg protocol.Message, boundary userInputBoundary) error {
	...
	} else if boundary == userInputStartsTurn && a.turnSafety.RequestContext != nil {
